...
class data:
    ...
    def eliminate_type_not_have(self,can_have_type_array):
        result = []
        for file in self.data:
            removed = 0
            for type in can_have_type_array:
                if file.split('.')[-1] != type:
                    print('remove: ' + file)
                    removed = 1
                    break
    ...
    def eliminate_fold(self):
        for i in self.data:
            print(i)
            print(i.split('\\')[-1])
            print(i.split('\\')[-1].__contains__('.'))
            print('======================')
            print(i.split('\\')[-1])
            if not i.split('\\')[-1].__contains__('.'):
                print('remove: ' + i)
                self.data.remove(i)